...
export default{
  ...
  methods: {
    ...
    leftClick(e,svgZuo,argument) {//点击听力图的函数
	  // debugger
	  ...
	  if (xmi === true && ymi === true) {
	    ...
	    this.chcekX(indexx, indexy,this.arrayyuanzuo, this.dataType2);
	    if(!(argument.zhu)){this.tableText(this.arrayyuanzuo, argument.tableText)};
	    this.yuan(svgZuo,this.arrayyuanzuo, argument.color, this.fontMask);
	    this.average(argument.average,this.arrayyuanzuo);
	  }
	},
	leftMove(e,svgZuo,argument,coord) {//鼠标移动时的函数
	  // console.log(e)
	  // debugger
	...
	rightClick(e,svgZuo,argument) {//点击听力图的函数
	  ...
	  if (xmi === true && ymi === true) {
	    ...
	    // 检查X轴点是否存在和此点坐标是否已经存在
	    this.chcekX(indexx, indexy,this.arrayyuan, this.dataType2)
	    if(!(argument.zhu)){this.tableText(this.arrayyuan, argument.tableText)}
	    this.yuan(svgZuo,this.arrayyuan, argument.color, this.fontMask)
	    this.average(argument.average,this.arrayyuan)
	  }
	},
	rightMove(e,svgZuo,argument,coord) {//鼠标移动时的函数
	  svgZuo.appendChild(this.gMove)
	  if(e.type == "touchmove" || e.type == "touchstart"){
	    var currentE = e.touches[0]
	  }else{
    ...
    _bigCanv(zy){//放大canvas
      ...
      if(zy === "zuo"){
        this.bigleft = true;
        svgNew.addEventListener("click",function(e){ vueD.leftClick(e,vueD.svgZuo,vueD.argument)},false);
        this.svgZuo = svgNew;
        beijing = 'rightBig';
      }else{
        this.bigleft = false;
        svgNew.addEventListener("click",function(e){ vueD.rightClick(e,vueD.svgYou,vueD.argumentR)},false);
        this.svgYou = svgNew;
        beijing = 'leftBig';
      }
    ...
    copy: function () {
      if (this.arrayyuan.length > this.arrayyuanzuo.length) {
        ...
        // this.clearTu('zuo1')
        this.yuan(this.svgZuo, this.arrayyuanzuo, '#0000FF', this.fontMask)
        this.average('L', this.arrayyuanzuo)
      } else {
        ...
        // this.clearTu('you1')
        this.yuan(this.svgYou, this.arrayyuan, '#FF0000', this.fontMask)
        this.average('R', this.arrayyuan)
      }
    },
    // 获取鼠标在canvas上的坐标
        	...
        	this.control_arr.forEach((item) =>{
        		if(objZong[v].type === item.key){
        			this.arrayyuan = this.ReadData(objZong[v].data);
          			this.yuan(this.svgYou, this.arrayyuan, '#FF0000', item.markRight);
        		}
        	})
        }
        // 生成左耳图
          	...
          	this.control_arr.forEach((item) =>{
        		if(objZong[v].type === item.key){
        			this.arrayyuanzuo = this.ReadData(objZong[v].data);
          			this.yuan(this.svgZuo, this.arrayyuanzuo, '#0000FF', item.markLeft);
        		}
        	})
        }
      };
      ...
      // 根据数据生成表中数据
        ...
        for (var v in objZong) {
          ...
          if (objZong[v].ears === '右') {
          	this.control_arr.forEach((item) =>{
        		if(item.value && objZong[v].type === item.key){
        			this.fontSign = item.markRight;
        			this.arrayyuan = this.ReadData(objZong[v].data);
        			// this.yuan1(this.svgYou, objZong[v].type, color1,str,this.arrayyuan)
        			this.yuan(this.svgYou,this.arrayyuan, color1,this.fontSign, objZong[v].type,str)
              		this.average('R', this.arrayyuan)
        		}
        	})
          }
          ...
          if (objZong[v].ears === '左') {
          	this.control_arr.forEach((item) =>{
        		if(item.value && objZong[v].type === item.key){
        			this.fontSign = item.markLeft;
        			this.arrayyuanzuo = this.ReadData(objZong[v].data);
        			// this.yuan1(this.svgZuo, objZong[v].type, color2,str,this.arrayyuanzuo)
        			this.yuan(this.svgZuo,this.arrayyuanzuo, color2,this.fontSign,objZong[v].type,str)
              		this.average('L', this.arrayyuanzuo)
        		}
        	})
          }
    ...
    drawyuan: function () {
      ...
      for (var v in objZong) {
        ...
        if (objZong[v].ears === '右') {
          	...
          	this.tableText(this.arrayyuan, 'r')
          	this.yuan(this.svgYou, this.arrayyuan, '#FF0000', this.fontSign + 'R')
          	this.average('R', this.arrayyuan)
        }
        // 生成左耳图
        if (objZong[v].ears === '左') {
          	...
          	this.tableText(this.arrayyuanzuo, 'l')
          	this.yuan(this.svgZuo, this.arrayyuanzuo, '#0000FF', this.fontSign + 'L')
          	this.average('L', this.arrayyuanzuo)
        }
      }
    },
    ...
    yuanshi: function () {
    	if(this.arrayyuanshi.length >0 || this.arrayyuanshizuo.length >0){
    		this.yuan(this.svgYou, this.arrayyuanshi, '#FF0000', this.fontSign + 'R');
    		this.average('R', this.arrayyuanshi)
    		this.yuan(this.svgZuo, this.arrayyuanshizuo, '#0000FF', this.fontSign + 'L');
    		this.average('L', this.arrayyuanshizuo);
    ...
    // 画图
    yuan1: function (svg, sing, color,str,arrayyuan) {
      this.gTu = this.createTag('g', {})
      this.gTu.id = svg.id + sing + str + 'tu'
      console.log(this.gTu.id)
      var fontOp
      for (var i = 0; i < arrayyuan.length; i++) {
        fontOp = this.fontSign
        if (arrayyuan[i][0] >= 20) {
          ...
          this.svgText = this.createTag('text', {
            'x': this.arrayXxiaCoordinate[(arrayyuan[i][0] - 20)] - 5, 'y': this.arrayYline2[(arrayyuan[i][1])] + 5, 'font-family': 'fonteditor'})
          ...
          this.svgText = this.createTag('text', {
            'x': this.arrayXcoordinate[(arrayyuan[i][0])] - 5, 'y': this.arrayYline2[(arrayyuan[i][1])] + 5, 'font-family': 'fonteditor'})
          this.svgText.textContent = fontarray[fontOp]
          this.svgText.style.fill = color
          this.gTu.appendChild(this.svgText)
        }
      }
      this.tinglixian(svg, this.gTu,arrayyuan,color)
    },
    // 画图
    yuan: function (svg, arrayL, color, mask,type,str) {
    	console.log(arguments)
      this.gTu = this.createTag('g', {})
      // this.gTu.id = svg.id+mask+ 'tu'
      let type1 = type ? type : "";
      let str1 = str ? str :"";
      let masks = str1 ? "" :mask;
      this.gTu.id = svg.id + masks + type1 + str1 + 'tu';
      console.log( this.gTu.id)
      console.log( svg.id)
      ...
      for (var i = 0; i < arrayL.length; i++) {
        ...
        if (arrayL[i][0] >= 20) {
          ...
          // console.log(fontOp)
          this.svgText = this.createTag('text', {
            'x': that.arrayXxiaCoordinate[(arrayL[i][0] - 20)] - that.markFontSize.top, 'y': that.arrayYline2[(arrayL[i][1])] + that.markFontSize.top, 'font-family': 'fonteditor','font-size':that.markFontSize.size,'font-weight':'bold'})
          this.svgText.textContent = fontarray[fontOp]
          this.svgText.style.fill = color
          this.gTu.appendChild(this.svgText);
        } else {
          ...
          this.svgText = this.createTag('text', {
            'x': that.arrayXcoordinate[(arrayL[i][0])] - that.markFontSize.top, 'y': that.arrayYline2[(arrayL[i][1])] + that.markFontSize.top, 'font-family': 'fonteditor','font-size':that.markFontSize.size,'font-weight':'bold'})
          this.svgText.textContent = fontarray[fontOp]
          this.svgText.style.fill = color
          this.gTu.appendChild(this.svgText);
        }
      }
      this.tinglixian(svg, this.gTu, arrayL, color)
    },
    // 听力线
