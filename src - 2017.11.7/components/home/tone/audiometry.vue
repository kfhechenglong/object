 	...
 	methods:{
 		...
 		change(evt){
 			...
 			if(this.changeToggle){
 				// this.volumeNum = -(6-evt)*10 + this.initialVolumeNum;
 				this.getVolumNum = (6-evt)*10;
 				let volumeNum = this.initialVolumeNum - this.getVolumNum;
 				// 保存音量差值
 				util._setVolumNum(this,{'decibel':this.getVolumNum,'test_id':sessionStorage.getItem('test_id')});
			    let argument = this.wskt.wstoctld('games_audio_toggle',{'volume':volumeNum});
...
//      保存综合评价的结果
 		...
 		toSendParams(){
	    	...
	    	let currentVolume = this.initialVolumeNum - this.getVolumNum;
	    	let params = {
