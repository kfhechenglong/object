    ...
    methods: {
      ...
      // 选择助听设备
        ...
        printMore(){//批量打印
            ...
            let that = this;
            function next(){
                that.$refs.checkprint.dialogToggleVisible = true;
            }
            // 终止打印任务开关
            this.noTask = true;
        },
        ...
        filterData(data){
            ...
            let dowork = function(item){
                ...
                return setSvgData (item.OtherData).then(() =>{
                    ...
                    return util.canvas2html('toimg',that);
                }).then(() =>{
                    // 查询打印任务
            ...
            // 音素测听
            let dowork_tone = function(item){
                return printDom(item)
                .then((str) =>{
                    if(that.type.key == 1){//林氏六音报告单
                        return that.checkedListData(str);
                    }else{//音素测听报告单
                        return that.checkedToneData(str);
                    }
                }).then(() =>{
                    return util.canvas2html('toimg',that);
                }).then(() =>{
            ...
            // function canvas2html(str){
                // return new Promise((resolve,reject) =>{
                //     let element = document.getElementById('toimg');
                //     var canvas = document.createElement('canvas');
                //     var context = canvas.getContext('2d');
                //     var scale = 2;//图片缩放;
                //     var w = element.offsetWidth + 150;
                //     var h = element.offsetHeight + 460;
                ...
                //     html2canvas(element, {height:1180,scale:2,canvas:canvas,onrendered:function(canvas){
                ...
                //            'time':'time'       
                //         }
                //         // that.imgurl.push(dataUrl)
                //         that.$ajax.post('/info/printbg',obj).then(function(response){
                //             console.log(response)
                //         }).catch((error) =>{
                //             alert(error + "报告保存出错")
                //         })
